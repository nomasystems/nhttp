...
# Generate per-vhost certificates for SNI tests
gen_vhost_cert() {
    local name="$1"
    local cn="$2"
    echo "Generating vhost certificate (${cn})..."
    openssl genrsa -out "$OUTPUT_DIR/${name}.key" "$KEY_SIZE" 2>/dev/null
    openssl req -new \
        -key "$OUTPUT_DIR/${name}.key" \
        -out "$OUTPUT_DIR/${name}.csr" \
        -subj "/O=$ORGANIZATION/CN=${cn}" \
        2>/dev/null
    cat > "$OUTPUT_DIR/${name}_ext.cnf" << EOF
basicConstraints=CA:FALSE
keyUsage=digitalSignature, keyEncipherment
extendedKeyUsage=serverAuth
...
EOF
    openssl x509 -req \
        ...
        -days "$DAYS" \
        -extfile "$OUTPUT_DIR/${name}_ext.cnf" \
        2>/dev/null
}

gen_vhost_cert "vhost_a" "a.test.local"
gen_vhost_cert "vhost_b" "b.test.local"
...
openssl verify -CAfile "$OUTPUT_DIR/ca.pem" "$OUTPUT_DIR/vhost_a.pem"
openssl verify -CAfile "$OUTPUT_DIR/ca.pem" "$OUTPUT_DIR/vhost_b.pem"
