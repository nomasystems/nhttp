...
%%%
%%% It contains **zero protocol logic**: every protocol effect goes
%%% through `m:nquic_lib' (and the same `m:nquic_session_ticket' glue
%%% the handshake gen_statem uses for post-handshake tickets). It is a
%%% faithful, minimal stand-in for a real owner such as
%%% `nhttp_conn_h3' / `nhttpc_h3_conn'.
%%%
...
-record(st, {
    ctx :: nquic:ctx(),
    parent :: pid(),
    socket :: nquic_socket:socket(),
    recv_waiters = #{} :: #{stream_id() => [#waiter{}]},
    accept_waiters = [] :: [#waiter{}],
    pending_streams = queue:new() :: queue:queue(stream_id()),
    datagrams = queue:new() :: queue:queue(binary()),
    %% Peer RESET_STREAM codes survive stream cleanup so a parked recv
    %% resolves to {error, {stream_reset, Code}} instead of the generic
    %% {error, stream_not_found} the protocol returns once the reset
    %% stream is gone. The H3 test client asserts on the code.
    resets = #{} :: #{stream_id() => non_neg_integer()},
...
-spec send(driver(), stream_id(), iodata()) -> ok | {error, term()}.
...
""".
-spec send(driver(), stream_id(), iodata(), timeout()) -> ok | {error, term()}.
...
-spec send_fin(driver(), stream_id(), iodata()) -> ok | {error, term()}.
...
-spec send_fin(driver(), stream_id(), iodata(), timeout()) -> ok | {error, term()}.
...
-spec recv(driver(), stream_id()) -> {ok, binary(), fin | nofin} | {error, term()}.
recv(Drv, StreamId) ->
    recv(Drv, StreamId, infinity).

-doc "Receive from a stream with a timeout. Mirrors `nquic:recv/3'.".
-spec recv(driver(), stream_id(), timeout()) ->
...
-spec open_stream(driver(), map()) -> {ok, stream_id()} | {error, term()}.
...
-spec is_writable(driver(), stream_id()) -> boolean().
...
init_accept(Parent, Listener, Opts) ->
    ...
    case nquic:accept(Listener, Opts) of
        {ok, Ctx0} ->
            ...
            %% Server-accepted connections stay on the dispatched
            %% path (takeover re-registered the CIDs to this owner);
            %% the listener's receivers (or, for `server_per_conn_fd',
            %% the per-connection receiver) forward `{packet, _}'.
            %% Upgrading here would open a second socket on the
            %% listener port and strand a migrated connection's
            %% replies. This mirrors `nhttp_conn_h3' (which only
            %% upgrades behind an explicit `connected_socket' opt).
            case nquic_lib:recv_pending(Ctx1) of
                {ok, Events, Ctx3} ->
                    ...
                    loop(arm_recv(St1));
                {error, Reason, Ctx3} ->
                    %% A peer violation in a packet buffered across the
                    %% handoff: emit the mapped CONNECTION_CLOSE before
                    %% exiting, the same RFC behaviour the handshake
                    %% gen_statem owned pre-handoff. Without this the
                    %% close is silently dropped (h3spec sees nothing).
...
-spec try_recv(stream_id(), from(), timeout(), #st{}) -> #st{}.
try_recv(StreamId, From, Timeout, #st{ctx = Ctx} = St) ->
    case nquic_lib:recv(Ctx, StreamId) of
        ...
        {error, Reason} ->
            reply(From, recv_error(StreamId, Reason, St)),
            St
    end.

-spec recv_error(stream_id(), term(), #st{}) -> {error, term()}.
recv_error(StreamId, Reason, #st{resets = R}) ->
    case maps:find(StreamId, R) of
        {ok, Code} -> {error, {stream_reset, Code}};
        error -> {error, Reason}
    end.

-spec park_recv(stream_id(), from(), timeout(), #st{}) -> #st{}.
...
-spec satisfy_recv_waiters(stream_id(), #st{}) -> #st{}.
...
-spec expire_recv_waiter(from(), stream_id(), #st{}) -> #st{}.
...
-spec satisfy_accept(stream_id(), #st{}) -> #st{}.
...
-spec queue_if_new(stream_id(), #st{}) -> #st{}.
...
%% `nquic_lib' orchestration with no protocol logic.
%%%-----------------------------------------------------------------------------

-spec bp_send(fin | nofin, stream_id(), iodata(), timeout(), from(), #st{}) -> #st{}.
bp_send(Fin, StreamId, Data, Timeout, From, St) ->
...
-spec backpressure_send(
    fin | nofin, stream_id(), binary(), integer() | infinity, #st{}
) -> {ok, #st{}} | {error, term(), #st{}}.
...
-spec lib_send(fin | nofin, nquic:ctx(), stream_id(), binary()) ->
...
process_events([{stream_reset, StreamId, Code} | Rest], #st{resets = R} = St) ->
    St1 = St#st{resets = R#{StreamId => Code}},
    process_events(Rest, satisfy_recv_waiters(StreamId, St1));
